...
describe('time-entry api', () => {
  ...
  const listTimeEntriesDirective = new ListTimeEntriesInMemoryDirective(repository.streamName, database)
  ...
  describe('endpoint /register-time-entry', () => {
    it('should register a time entry', async () => {
      ...
      const events = await eventStore.load(repository.streamName, id)
    ...
    it.each([
      {
        __scenario: 'MISSING_UUID',
        payload: { userId: '', startTime: subHours(now, 1).getTime(), endTime: now.getTime() },
      },
      ...
      {
        __scenario: 'INVALID_UUID',
        payload: { userId: 'invalid-uuid-id', startTime: subHours(now, 1).getTime(), endTime: now.getTime() },
      },
      {
        __scenario: 'INVALID_START_TIME',
        payload: { userId: uuidv7(), startTime: subHours(now, 1).toTimeString(), endTime: now.getTime() },
      },
      {
        __scenario: 'INVALID_END_TIME',
        payload: { userId: uuidv7(), startTime: subHours(now, 1).getTime(), endTime: now.toUTCString() },
      },
  ...
  describe('endpoint /list-time-entries', () => {
    ...
    beforeAll(async () => {
      await Promise.all(documents.map(async (document) => {
        ...
        await database.execute(repository.streamName, statement)
      }))
    })
