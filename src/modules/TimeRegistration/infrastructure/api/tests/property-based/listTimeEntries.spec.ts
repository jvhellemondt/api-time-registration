...
import { listTimeEntriesByUserIdPayload } from '@modules/TimeRegistration/usecases/queries/ListTimeEntries/ListTimeEntries.ports.ts'
...
describe('request.GET /list-time-entries', () => {
  ...
  it('lists time entries or rejects invalid payloads', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          userId: fc.oneof(fc.string({
            minLength: 0,
            maxLength: 50,
          }), fc.constant(''), fc.constant('invalid-uuid'), fc.uuid()),
        }),
        async (payload) => {
