...
} from '@/infrastructure/database/in-memory/directives/StoreTimeEntries/StoreTimeEntries.in-memory.directive.ts'
...
describe('in-memory UpdateTimeEntriesDirective', () => {
  ...
  it.each([
    ...
    const updateStatement = { id: documents.filter(({ userId }) => userId === user.id)[0].id, startTime: subHours(new Date(), 5).toISOString() }
